...
def MLP(channels:list, batch_norm:bool=True, last_layer:bool=False, drop_out:bool=False):
    layers = list()
...
class MLP_V2(pl.LightningModule):
    def __init__(self, channels:list, norm_type:str=None, transpose_input:bool=False):
...
class EquivariantLayer(nn.Module):
    def __init__(self, num_in_channels:int, num_out_channels:int, activation='relu', normalization=None, momentum:float=0.1,
                 num_groups:int=16):
        ...
        elif 'layer' == self.normalization:
            self.norm = nn.LayerNorm(self.num_out_channels)
